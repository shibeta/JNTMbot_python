# JNTMbot_python - GTAOL 德瑞差事自动化脚本
...
- [🔬 项目架构](#-项目架构)
- [🤝 交流与反馈](#-交流与反馈)
    - [1. 遇到使用问题或环境配置疑问？](#1-遇到使用问题或环境配置疑问)
    - [2. 发现 Bug 或想要新功能？](#2-发现-bug-或想要新功能)
    - [3. 想要贡献代码？](#3-想要贡献代码)
- [🙏 致谢](#-致谢)
- [📄 许可证](#-许可证)
...
5.  **修改环境设置**
    ...
    修改游戏内和游戏外的部分设置以适配自动化程序。详细说明请见 [使用说明](#2-环境设置) 部分。
...
2.  **备份配置文件**
    ...
    > 大多数情况下，您可以直接用旧的配置文件覆盖新文件。程序会自动向旧配置文件中添加缺少的项。
    >
    > 如果用旧的配置文件覆盖新文件后，程序工作不正常，请改为使用最新的配置文件。
...
为确保 Bot 能正常工作，请完成以下设置：

1. **游戏外设置**

- ✅ 确保 **没有** 连接 **任何游戏手柄** 到电脑 (与虚拟手柄冲突)。
- ✅ 确保没有安装 **PlayStation®Accessories** (与虚拟手柄冲突)。
- ✅ 确保没有开启 **[Windows 自动 HDR](https://go.microsoft.com/fwlink/?linkid=2159313)** 。或者，在 **Windows 设置 > 系统 > 屏幕 > 显示卡** 中单独关闭 GTA5 的自动 HDR (影响游戏窗口内容识别)。
- ✅ 确保将 **Steam 设置 > 通知 > 显示弹出通知...** 设置为 **从不** (影响游戏窗口内容识别)。
- ✅ 确保将 **Steam 设置 > 游戏中 > 显示性能监视器** 设置为 **关闭** 。或者，将 **Steam 库 > GRAND THEFT AUTO V 增强版 > 属性 > 通用 > 在游戏中启用 Steam 叠加界面** 设置为 **关闭** (影响游戏窗口内容识别)。
- ✴️ 可选: 将 **RockStar Games Launcher > 设置 > 禁用 Windows 键** 设置为 **关闭** (与控制热键冲突，不需要使用热键可以不改)。
...
- ✅ 确保已经作为 **房主** 完成过 **至少一次** 德瑞差事终章 (避免首次任务需要等待富兰克林电话)。
...
- 请根据页面引导，选择对应的模板（Bug 反馈 / 新功能建议）并完整填写。对于 Bug 反馈，提供上下文详尽的**控制台日志**和发生问题时的**游戏完整截图**将极大加快排查速度。
