...
- **🛡️ 智能故障处理**：当Bot长时间无响应时，可配置通过 PushPlus 发送微信警报并安全退出，保障账号安全。
...
4.  **修改配置 (可选)**
    根据您的需求，编辑根目录下的 `config.yaml` 文件。详细说明请见 [使用说明](#️-使用说明) 部分。
...
-   `steam_bot/server.js`: 基于 `node-steam-user` 的 Node.js 后端，将 Steam Chat 功能封装为 HTTP API。
